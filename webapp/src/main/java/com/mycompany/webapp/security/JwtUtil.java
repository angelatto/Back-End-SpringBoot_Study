...
public class JwtUtil {
	...
	public static String createToken(String uid) {
		String token = null;
		try {
			token = Jwts.builder().setHeaderParam("typ", "JWT") // 토큰의 종류
					.setHeaderParam("alg", "HS256") // 암호화 알고리즘 종류
					.setExpiration(new Date(new Date().getTime() + 1000 * 60 * 60 * 12)) // 토큰의 유효기간, 12시간 
	...
	public static boolean validateToken(String token) {
		...
		try {
			JwtParser parser = Jwts.parser();
			parser.setSigningKey(secretKey.getBytes("UTF-8")); // 비밀키 배열로 줘야함
			Jws<Claims> jws = parser.parseClaimsJws(token);
			Claims claims = jws.getBody();
			validate = claims.getExpiration().after(new Date()); // after, !before 
			if (validate) {// 만료시간이 현재 시간보다 after(미래시간) 만료안됨.
